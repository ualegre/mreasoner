package edu.casetools.mreasoner.deployment.simulation;
...
public class EventSimulator extends Thread {
	...
	private int compareTimes(Event event){
		...
		c1.setTimeInMillis(AbstractMReasoner.systemStatus.getTime().getSystemRealTime());
		c2.setTimeInMillis(AbstractMReasoner.systemStatus.getTime().getSystemRealTime());
	...
	//	if(time.isSimulation()){
			AbstractMReasoner.semaphore.inputTake();
				...
				for(int i=0;i<events.size();i++){
					...
					AbstractMReasoner.systemStatus.occurs(events.get(i).getName(), events.get(i).getStatus(), true);
